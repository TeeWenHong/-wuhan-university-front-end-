<template>
  ...
  <div v-for="(top, index) in displayedBooks" :key="index">
    ...
    <div class="book-info">
      <!-- <h2>路径{{ top.picture_path }}</h2><br> -->
      <h2>{{ top.bookname}}</h2>
      <h5>类型：{{ top.tag}}</h5>
      <!-- <h5>书籍位置：{{ top.user_id }}</h5> -->
      <p>作者： {{ top.username }}</p>
      <!-- <p>pic_id： {{ top.pic_id }}</p><br> -->
  ...
  export default {
    ...
    data(){
      return{
        ...
        this.$http.get('/searchBookNamees?val='+this.test).then(response => {
        ...
        for (let i = 0; i < response.data.length; i++) {
            ...
            this.$http.get('/searchIdPices?user_id='+this.top[i].user_id).then(response => {
